...
# remember to change this later to fit matrix ??
...
BTN_2 = Pin(8, Pin.IN, Pin.PULL_UP)
BTN_3 = Pin(9, Pin.IN, Pin.PULL_UP)
BTN_4 = Pin(10, Pin.IN, Pin.PULL_UP)
BTN_5 = Pin(11, Pin.IN, Pin.PULL_UP)
BTN_6 = Pin(12, Pin.IN, Pin.PULL_UP)
BTN_7 = Pin(13, Pin.IN, Pin.PULL_UP)
BTN_8 = Pin(14, Pin.IN, Pin.PULL_UP)
BTN_9 = Pin(15, Pin.IN, Pin.PULL_UP)

BTN_POWER = Pin(16, Pin.IN, Pin.PULL_UP)
BTN_MENU = Pin(17, Pin.IN, Pin.PULL_UP)
BTN_UNDO = Pin(18, Pin.IN, Pin.PULL_UP)
BTN_PENCIL = Pin(19, Pin.IN, Pin.PULL_UP)
...
class ButtonState:
    ...
    def update(self):
        ...
        if val == 0 and self.last_val == 1:   # falling edge
            self.pressed = True
            self.held_at = now
        elif val == 1 and self.last_val == 0:  # rising edge
            if self.pressed:
                duration = time.ticks_diff(now, self.held_at)
                event = 'hold' if duration >= HOLD_MS else 'tap'
            self.pressed = False
            self.held_at = None
        self.last_val = val
        return event
...
def main():
    ...
    buttons = {
        'up':     ButtonState(BTN_UP),
        'down':   ButtonState(BTN_DOWN),
        'left':   ButtonState(BTN_LEFT),
        'right':  ButtonState(BTN_RIGHT),
        'select': ButtonState(BTN_SELECT),
        ...
        'power': ButtonState(BTN_POWER),
        'menu': ButtonState(BTN_MENU),
        'undo': ButtonState(BTN_UNDO),
        'pencil': ButtonState(BTN_PENCIL)
    }

    # Initial draw
    render(epd, game)
    dirty = False # whether game has changed to render updates
    ...
    while True:
        ...
        up_ev     = buttons['up'].update()
        down_ev   = buttons['down'].update()
        left_ev   = buttons['left'].update()
        right_ev  = buttons['right'].update()
        select_ev = buttons['select'].update()

        num1_ev = buttons['1'].update()
        num2_ev = buttons['2'].update()
        num3_ev = buttons['3'].update()
        num4_ev = buttons['4'].update()
        num5_ev = buttons['5'].update()
        num6_ev = buttons['6'].update()
        num7_ev = buttons['7'].update()
        num8_ev = buttons['8'].update()
        num9_ev = buttons['9'].update()

        power_ev = buttons['power'].update()
        menu_ev = buttons['menu'].update()
        undo_ev = buttons['undo'].update()
        pencil_ev = buttons['pencil'].update()
        ...
        if up_ev == 'tap':
            ...
            if game.pencil:
                game.jot(1); dirty = True
            else:
                game.input(1); dirty = True
            print("1")
        elif num2_ev == 'tap':
            if game.pencil:
                game.jot(2); dirty = True
            else:
                game.input(2); dirty = True
            print("2")
        elif num3_ev == 'tap':
            if game.pencil:
                game.jot(3); dirty = True
            else:
                game.input(3); dirty = True
            print("3")
        elif num4_ev == 'tap':
            if game.pencil:
                game.jot(4); dirty = True
            else:
                game.input(4); dirty = True
            print("4")
        elif num5_ev == 'tap':
            if game.pencil:
                game.jot(5); dirty = True
            else:
                game.input(5); dirty = True
            print("5")
        elif num6_ev == 'tap':
            if game.pencil:
                game.jot(6); dirty = True
            else:
                game.input(6); dirty = True
            print("6")
        elif num7_ev == 'tap':
            if game.pencil:
                game.jot(7); dirty = True
            else:
                game.input(7); dirty = True
            print("7")
        elif num8_ev == 'tap':
            if game.pencil:
                game.jot(8); dirty = True
            else:
                game.input(8); dirty = True
            print("8")
        elif num9_ev == 'tap':
            if game.pencil:
                game.jot(9); dirty = True
            else:
                game.input(9); dirty = True
            print("9")
            ...
            if game.menu_open:
                ...
                dirty = True  # redraw game when closing menu
            print("menu")
        elif undo_ev == 'tap':
